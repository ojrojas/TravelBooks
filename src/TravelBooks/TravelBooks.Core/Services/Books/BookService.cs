...
public class BookService : IBookService
{
    ...
    {
        CreateBookResponse response = new(request.Correlation);
    ...
    {
        DeleteBookResponse response = new(request.Correlation);
    ...
    {
        UpdateBookResponse response = new(request.Correlation);
        if (request.Book is null) throw new ArgumentNullException(nameof(request));
        _logger.LogInformation($"Create author {JsonSerializer.Serialize(request.Book)}");
    ...
    {
        ListBooksResponse response = new(request.Correlation);
